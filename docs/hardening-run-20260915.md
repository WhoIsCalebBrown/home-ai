...
## 2026-09-15 continuation: bridge failure semantics and deployment recovery

- Tools executor transport failures now return structured `BRIDGE_UNAVAILABLE` with `submission_transport_success=false`, `ingestion_confirmed=false`, and a consumed confirmation; they no longer re-raise through the tool boundary. The isolated suite reached 144 passing tests, including replay rejection after a simulated connection failure.
...
- The provenance classifier was refined to use container Compose labels and template ownership evidence. The current report has zero unresolved owner gaps; one explicit third-party policy review remains for the locally tagged Faster-Whisper image (`voice-wyoming-whisper:20260913`), whose Unraid template declares the upstream OHF-Voice project. The custom LifeOS, Music Enricher, and NMPZ Compose builds are reported as local/Compose-managed rather than falsely treated as upstream image violations.

## 2026-09-15 continuation: total live-tool failure hardening

- Observation: Tools already returned distinct transport statuses, but the Assistant could still fall through to Qwen after an all-failed live read. That permits a fluent answer unsupported by live evidence.
- Hypothesis: the missing boundary was not backend classification; it was the absence of a deterministic “no evidence, no synthesis” guard in the Assistant response path.
- Experiment/control: added `all_live_results_failed()` and exercised timeout, HTTP 500, malformed result, and successful-empty-result controls in the isolated lane. A total failure now yields a truthful unavailable response; a successful empty result remains available for semantic “zero results” handling.
- Fix: Tools failures now carry non-secret `error_code`, `retryable`, and `evidence_available=false` fields (`TIMEOUT`, `BACKEND_UNAVAILABLE`, `EXECUTION_FAILED`, `INVALID_TOOL_RESULT`). Assistant refuses Qwen synthesis when every attempted live tool failed.
- Result: isolated safety suite is 148/148. First CI run correctly failed because the new tests wrote to production `/data` under the GitHub runner; the fixture was corrected to use pytest `tmp_path`, then the replacement CI run passed Assistant tests and remained in progress for image publication at checkpoint time. No production service or media state was changed by this work.
- Falsification still pending: deploy the new owned Assistant/Tools images, repeat production smoke, and run the read-only audio control lane with an injected/fake outage rather than only unit-level failure fixtures.

## 2026-09-15 continuation: post-deploy voice falsification

- Assistant `sha-92a7ea5` and Tools `sha-92a7ea5` were deployed from CI-validated artifacts; the Unraid templates were updated to the same pins. Assistant/Tools stayed on `voiceai`, `server-tools` DNS alias remained present, third-party services were not restarted, and the rollback backup is `/mnt/cache/appdata/home-ai/deploy-backups/20260915-1200`.
- Production smoke after deployment: Assistant READY, Tools 68 capabilities, contract 1.0, Qwen `qwen3.5:4b`/8192 unchanged, Pocket TTS unchanged, Frigate reachable, drift empty.

## Continued voice hardening checkpoint

## 2026-09-15 continued long-run qualification

- The expanded live audio qualification completed 102 multi-turn conversations / 204 real audio turns. It exited non-zero because the harness found genuine routing candidates; this was treated as a falsification result, not a pass. Cross-domain controls and direct-file safety remained successful, while failures clustered around ASR-damaged media status and recency phrasing.
- Reproduction: “Where's Dumb and Dumber?” could fall through to Qwen without a bounded read tool, and “it's new in Plex” could select generic plex_search. A first parser change was deliberately falsified by the existing suite: it disturbed weather/server follow-up routing and the matrix caught five regressions. The fix was narrowed to explicit-domain ordering and title-shaped status evidence.
- aeeda88 added bounded standalone media-status and Plex-recency repairs; CI run 34974218596 passed, Assistant sha-aeeda88 was deployed with a timestamped rollback container/template backup, and health/contract/drift/read-only smoke passed. Live reproduction then confirmed the standalone title and Plex recency fixes.
- A remaining live case, “That's the hobby, ready?”, showed Whisper title corruption. The Assistant did not call a weather or write-capable tool and returned a bounded no-matching-workflow status. The audio harness now classifies this as safe_stt_recovery, not a routing success; no title-specific alias was added.
- 1071a0c moved title-shaped media-domain precedence after explicit weather/server/web markers and added a regression. CI run 34974839381 passed. Assistant sha-1071a0c is now live; Tools remains sha-92a7ea5. Post-deploy Assistant health is READY, Tools has 68 capabilities, contract validation passes, Unraid drift is empty, and the targeted audio controls are clean.
- Current isolated evidence: 154 tests passed; conversation matrix 31 scenarios / 12 confirmation forms passed; the targeted real-audio controls passed with safe classification for the remaining ASR-corrupted title. No production media write, third-party image change, or manager/provider mutation occurred.
- Remaining P1/P2: full post-sha-1071a0c 204-turn rerun, longer read-only soak, broader fault injection, and stronger canonical-identity confidence scoring for severe title corruption. Standard season gate remains false; permanent and music writes remain outside this pass.

- `48ab2f8` added a context-bound retained-workflow repair for ASR-damaged media status turns. It cannot create a workflow, write, or override an explicit web/weather/camera/server domain. `070922a` rejected weather/time framing words such as `weather` and `today` as literal locations, preventing a malformed transcript from poisoning the retained home location. `5c0c1b7` classified truthful read-backend failures and safe STT clarifications separately in the real audio harness. `cfa6521` broadened the retained repair to equivalent `that was/is` and `it was/is` ASR shapes after a live falsification exposed `That was Dumb and Dumberdorn`.
- Isolated QA passed `151` tests. Live audio controls after deployment: retained-workflow ASR repair passed; camera→web domain switching passed concurrently; weather→server switching passed; concurrent isolated sessions had no cross-session referent leakage. A transient live weather backend error was classified as `backend_read_failure_truthful`, not a routing failure, and the Assistant did not invent weather facts.
- Production final Assistant image: `ghcr.io/whoiscalebbrown/home-ai-assistant:sha-cfa6521`; Tools remains `ghcr.io/whoiscalebbrown/home-ai-tools:sha-92a7ea5`. Unraid template and runtime drift report are empty; management labels were restored on the Assistant container. The machine-readable contract was refreshed in `6fc0879`.
- Live read-only workflow inventory: Dumb and Dumber TMDB 8467 is `REQUESTED`; The Hobbit TMDB 1362 is `SEARCHING`; no 10th Kingdom workflow record was present in the persisted Home-AI workflow store at inspection time. No new media write was issued during this pass.
- Remaining P1 evidence gaps: longer read-only soak, richer canonical title repair for noisy but non-destructive media status transcripts, and broader TV/anime/music lifecycle observability. These remain intentionally open; no production media-write qualification is claimed.
- Full post-deploy real-audio conversation run completed without harness errors across the checked-in multi-turn catalog. It confirmed media/status, web, Frigate historical/live, weather, Plex, containers, and cross-domain isolation through the live WebSocket/STT/TTS lane. Read-only only.
- Full read-only audio scenario sweep deliberately falsified the harness: severe STT collapse `Dumb and Dumber`→`Dumberdorn` lost the media tool route, and a playback request degraded to “a movie inside this conversation” and selected Plex. The latter received a generic playback classifier fix; the former remains an STT limitation and is not repaired with a title-specific alias.
- The playback fix was deployed as Assistant `sha-5c67ebc`, then verified through the real audio lane: degraded “Play the movie inside this conversation” produced the unsupported-playback response with no tool selected. Tools remained `sha-92a7ea5`; no media write occurred.
- New evidence gap: the corpus permits expected-tool checks but does not yet assert canonical identity quality for every STT mutation. Weather variants produced `Todaya`/`Welling` in some runs while still selecting the weather tool; this remains a P1 entity-confidence/repair issue, not a safe reason to broaden aliases.

## 2026-09-15 continuation: bounded audio harness and event-scoped Frigate evidence

- The long-run audio harness was observed waiting without visible progress. Static inspection found no flushed per-conversation progress and no all-conversation watchdog. The harness now bounds WebSocket open/send operations, applies a 240-second per-conversation timeout, emits each completed conversation immediately, and classifies a watchdog expiry as `harness_timeout` rather than silently hanging.
- A focused real-audio control falsified the assumption that historical visual follow-ups were already event-bound: `What were they wearing?` was being re-routed to `frigate_recent_events` after follow-up normalization. The generic fix gives normalized `for event <id>` scopes precedence over broad historical search. Isolated QA reached 156 passing tests.
- Assistant-only deployment of CI-validated `sha-3ed90e4` completed successfully. Rollback template: `/mnt/cache/appdata/home-ai/deploy-backups/20260915T134534Z-Home-AI-Assistant.xml`; rollback image tag: `ghcr.io/whoiscalebbrown/home-ai-assistant:rollback-1071a0c-before-3ed90e4`. Tools, cli_debrid, Frigate, Plex, Qwen, and TTS were not restarted or modified.
- Post-deploy evidence: Assistant READY, Tools READY with 68 capabilities, contract 1.0, runtime/template drift empty, Qwen `qwen3.5:4b`/8192 and Pocket TTS unchanged. Live audio passed historical events → `frigate_event_snapshot` → live snapshot, and a separate activity conversation passed events → `frigate_event_activity` → live snapshot. No media write occurred.
- QA oracle update `172db4a` records event-scoped visual evidence and the activity conversation. The broad corpus remains intentionally paused; its prior incomplete run is not counted as a qualification pass. Remaining P1 gaps are the full post-deploy corpus, longer soak, fault-injection expansion, and canonical identity scoring for noisy STT.

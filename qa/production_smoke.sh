...
token=open(\"/run/secrets/home-ai-tools-token\").read().strip()
headers={\"X-Home-AI-Tools-Token\":token}
print(\"assistant_health=\"+urllib.request.urlopen(\"http://127.0.0.1:8088/health\",timeout=10).read().decode())
print(\"tools_health=\"+urllib.request.urlopen(urllib.request.Request(base+\"/health\",headers=headers),timeout=10).read().decode())
for q in (\"how is Dumb and Dumber doing\", \"what happened today in American politics\", \"describe the image from that detection\"):
    ...
    print(\"discover=\"+q+\" -> \"+urllib.request.urlopen(urllib.request.Request(url,headers=headers),timeout=10).read().decode()[:600])
for name,args in ((\"list_containers\",{\"status\":\"running\"}), (\"media_storage_status\",{\"media_type\":\"movie\"}), (\"frigate_status\",{})):
    ...
    req=urllib.request.Request(base+\"/invoke\",data=json.dumps(body).encode(),headers={**headers,\"Content-Type\":\"application/json\"})
